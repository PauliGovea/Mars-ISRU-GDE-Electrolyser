...
```text
├── data/
│   ├── raw/                      # Unmodified instrument exports
│   │   ├── gc_data/              # GC peak areas and concentrations (CompactGC, Agilent 990)
│   │   ├── power_supply/         # EA Power Control logs (delivered current and cell voltage)
│   │   └── flow/                 # BPC Go flow meter exports (cathode and anode)
│   └── processed/                # Per-step data summaries generated by the Jupyter notebooks
├── notebooks/
│   ├── Na2SO4/                   # Individual notebooks per condition: 0.3-0.9 M, pH 1-3 (2 h steps)
│   ├── K2SO4/                    # Individual notebooks per condition: 0.3-0.9 M, pH 1-3 (2 h steps)
│   ├── All_Na2SO4_merged/        # Analysis of all merged Na2SO4 baseline datasets
│   ├── Merged_and_Best_Condition/# Combines all summaries for Figs 3 & 4 (0.6 M K2SO4 pH 3: mass, energy, MOXIE comparison)
│   └── anode_O2/                 # Anode gas composition and FE(O2) analysis (20 min steps)
├── figures/                      # Publication-ready figures alongside their generation code
├── tables/                       # Supplementary Tables S1-S5 in CSV format (located in 'All K2SO4 merged')
├── requirements.txt              # Python package versions required to reproduce the environment
├── LICENSE
└── README.md
```
